...
def other_input():
    ...
    if Suggest == 4:
        suggestl = [[Text("Suggestions:", font=font)],
                    [Button(stable[0], font=font),Button(stable[1], font=font)],
                    [Button(stable[2], font=font),Button(stable[3], font=font)],
                    [Text("Πληκτρολογήστε γιατί: (Optional)", font=font)],
                    [InputText(font=font)],
                    [Button("Submit", font=font)]]
        window2 = Window("Other", suggestl, icon=r'C:\popicon.ico')
        while 1:
            ...
            # values = values['-IN-']
                ...
                sheet1.update_cell(row, coll + 5, stable[0])
                window2.close()
                break
            elif event == stable[1]:
                sheet1.update_cell(row, coll + 5, stable[1])
                window2.close()
                break
            elif event == stable[2]:
                sheet1.update_cell(row, coll + 5, stable[2])
                window2.close()
                break
            elif event == stable[3]:
                sheet1.update_cell(row, coll + 5, stable[3])
                window2.close()
                break
            window2.close()
...
def change_theme():
    layout5 = layout = [[Button('Μπλεδούλι🔵', font=font)],
...
def get_from_name(nameid):
    if nameid == 105:  # user1
        num = 7
    elif nameid == 118:  # user2
        num = 14
    elif nameid == 106:  # user3
        num = 21
...
#row = get_from_chell()  # edo einai to problima

google_sh = gc.open("pop-popup debugging data")
lsettings = google_sh.worksheet("Lunch Settings")
FailSafe = lsettings.acell('A2').value
FailSafe = int(FailSafe)
Testing = lsettings.acell('B2').value
Testing = int(Testing)
DName = lsettings.acell('C2').value
SName = lsettings.acell('D2').value
Suggest = lsettings.acell('E2').value
Suggest = int(Suggest)

stable = [None, None, None, None, None, None]

i = 0
while i != Suggest:
    stable[i] = lsettings.cell(i + 3, 5).value
    i += 1
...
if user_id == 1955:
    ...
    if Testing == 0:
        google_sh = gc.open(DName)
        sheet1 = google_sh.worksheet(SName)
    elif Testing == 1:
        google_sh = gc.open("tost")
        sheet1 = google_sh.worksheet("Sheet1")
    greedgeexaltedones()
